...
class UserController extends Controller
{
    ...
    //Dashboard
        public function dashboard()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_dashboard");
            
            $data['title']=$set->site_name.' Dashboard';
            
            $data['n_transactions']= Transactions::where("receiver_id", Auth::guard('user')->user()->id)->wherestatus(1)->count('*');
            $data['n_avgticket']= Invoice::where("user_id", Auth::guard('user')->user()->id)->wherestatus(1)->avg('total');
            $data['n_transactions_total']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('amount', '!=', null)->wherestatus(1)->sum('amount');
            $data['n_transactions_pix']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('payment_type', 'pix')->wherestatus(1)->sum('amount');
            $data['n_transactions_creditcard']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('payment_type', 'creditcard')->wherestatus(1)->sum('amount');
            $data['n_transactions_boleto']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('payment_type', 'boleto')->wherestatus(1)->sum('amount');
            
            $data['n_transactions_pix'] += Order::where("seller_id",Auth::guard('user')->user()->id)->where('payment_type', 'pix')->wherestatus(1)->sum('total');
            $data['n_transactions_creditcard'] += Order::where("seller_id",Auth::guard('user')->user()->id)->where('payment_type', 'creditcard')->wherestatus(1)->sum('total');
            $data['n_transactions_boleto'] += Order::where("seller_id",Auth::guard('user')->user()->id)->where('payment_type', 'boleto')->wherestatus(1)->sum('total');

            $data['n_transactions_visa']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('creditcard_brand', 'VISA')->where('payment_type', 'creditcard')->wherestatus(1)->sum('amount');
            $data['n_transactions_mastercard']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('creditcard_brand', 'MASTER')->where('payment_type', 'creditcard')->wherestatus(1)->sum('amount');
            $data['n_transactions_elo']=Transactions::where("receiver_id",Auth::guard('user')->user()->id)->where('creditcard_brand', 'ELO')->where('payment_type', 'creditcard')->wherestatus(1)->sum('amount');
            
            $data['n_transactions_visa'] += Order::where("seller_id",Auth::guard('user')->user()->id)->where('creditcard_brand', 'VISA')->where('payment_type', 'creditcard')->wherestatus(1)->sum('total');
            $data['n_transactions_mastercard'] += Order::where("seller_id",Auth::guard('user')->user()->id)->where('creditcard_brand', 'MASTER')->where('payment_type', 'creditcard')->wherestatus(1)->sum('total');
            $data['n_transactions_elo'] += Order::where("seller_id",Auth::guard('user')->user()->id)->where('creditcard_brand', 'ELO')->where('payment_type', 'creditcard')->wherestatus(1)->sum('total');
            

            $data['n_paid']= Transactions::where("receiver_id", Auth::guard('user')->user()->id)->wherestatus(1)->count('*');
            $data['n_pending']= Transactions::where("receiver_id", Auth::guard('user')->user()->id)->wherestatus(0)->count('*');
            $data['n_defaulter']= Invoice::where("user_id", Auth::guard('user')->user()->id)->where('due_date', '<', date("Y-m-d H:i:s"))->wherestatus(0)->count('*');
            ...
            $data['pendingBalance'] = PendingBalance::whereuser_id(Auth::guard('user')->user()->id)->where("liquidated", 0)->sum('amount');
            $data['pendingBalanceList'] = PendingBalance::whereuser_id(Auth::guard('user')->user()->id)->where("liquidated", 0)->orderBy("liquidation_date")->get();
        ...
        {
            $data['lang'] = parent::getLanguageVars("user_search_pages");
            $data['title'] = "Search Result";
            $data["userTax"] =  UserTax::getUserTax(Auth::guard('user')->user()->id);
    ...
    //Audit log
        public function audit()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_profile_pages");
        ...
        public function submitbill(Request $request){
            ...
            if($user->balance>$request->amount){
                ...
                if (array_key_exists('data', $result) && ($result['status'] === 'success')) {
                    ...
                    $charge['user_id']=$user->id;
                    $charge['ref_id']=$trx;
                    $charge['amount']=$request->amount*$userTax->bill_charge/100+($userTax->bill_chargep);
                    $charge['log']='Bill payment for #'.$trx;
                    Charges::create($charge);

                    $his['user_id']=$user->id;
                    $his['amount']=$request->amount+($request->amount*$userTax->bill_charge/100+($userTax->bill_chargep));
                    $his['ref']=$trx;
                    $his['main']=1;
                    $his['type']=2;
                    History::create($his);
                    ...
                    //Debit User
                    $user->balance=$user->balance-$request->amount-($request->amount*$userTax->bill_charge/100+($userTax->bill_chargep));
                    $user->save();
                    ...
                    $sav['amount']=$request->amount;
                    $sav['biller']=$ebiller;
                    $sav['charge']=$request->amount*$userTax->bill_charge/100+($userTax->bill_chargep);
                    $sav['ref']=$trx;
    ...
    //Support ticket
        public function ticket()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_support_pages");
            $data['title']='Tickets';
            ...
            if($request->hasfile('image')){
                ...
                if ($validator->fails()) {
                    ...
                    foreach($request->file('image') as $file){
                        ...
                        $savedFiles[] = $name;  
                        $sav['files'] = json_encode($savedFiles);
                    }
                }
            }
            $set=Settings::first();
    ...
    //Store
        public function product()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_product_pages");
            $data['title']='Products';
        ...
        public function submitstore(Request $request)
        {
            ...
            if($check>0){
                ...
                $sav['store_url']= str_replace(" ", "-", strtolower($request->store_name));
        ...
        {            
            ...
            $store->store_url = str_replace(" ", "-", strtolower($request->store_name));
            $store->save();
        ...
        public function ask($id) {
            ...
            $userTax = UserTax::getUserTax($product->user_id);
        ...
        public function cardpay($id) {
            ...
            $data["id"] = $product->ref_id;

            return redirect()->route('product.link', $data);
        }        
        ...
        public function pixpay($id) {

            $product = $data['product']=Product::whereref_id($id)->first();
            Session::put('pay-type', 'pix');
            return redirect()->route('product.link', ['id'=>$product->ref_id]);
        }        
        
        public function boletopay($id) {

            $product = $data['product']=Product::whereref_id($id)->first();
            Session::put('pay-type', 'boleto');
            return redirect()->route('product.link', ['id'=>$product->ref_id]);
        }       
        ...
        public function sask($id)
        {
            ...
            $data['title']='Payment Method';
            $data['cart']=$id;
            $product = Cart::whereuniqueid($id)->first();
            $store =Storefront::whereid($product->store)->first();

            $userTax = UserTax::getUserTax($store->user_id);
            $data['userTax']=$userTax;
        ...
        public function sboletopay($id) {
            Session::put('pay-type', 'boleto');
            return redirect()->route('checkout', ['id'=>$id]);
        }       
        
        public function spixpay($id) {
            Session::put('pay-type', 'pix');
            return redirect()->route('checkout', ['id'=>$id]);
        } 
        ...
        public function checkout($id) {
            ...
            $products = $data['products']=Cart::whereuniqueid($id)->count("*");
        ...
        {
            $data['lang'] = parent::getShopVars("user_product_pages");
        ...
        public function stripeacquireproduct(Request $request, $id)
        {
            ...
            $userTax = UserTax::getUserTax($merchant->user_id);
            try {
                $charge=$stripe->paymentIntents->retrieve($request->input('payment_intent'));
                if ($charge['status']=="succeeded") {
                    $up_mer->balance=$total+$up_mer->balance-($total*$userTax->product_charge/100+($userTax->product_chargep));
                    $up_mer->save();
                    ...
                    $sav['charge']=$charge=($total*$userTax->product_charge/100+($userTax->product_chargep));
                    ...
                    $chargex['amount']=$total*$userTax->product_charge/100+($userTax->product_chargep);
                    $chargex['log']='Received payment for order #' .$token;
                    Charges::create($chargex);
                    $his['user_id']=$up_mer->id;
                    $his['amount']=$total-($total*$userTax->product_charge/100+($userTax->product_chargep));
                    $his['ref']=$token;
                    $his['main']=1;
                    $his['type']=1;
                    $his['stripe_id']=$charge['id'];
                    $his['charge']=$total*$userTax->product_charge/100+($userTax->product_chargep);
                    History::create($his);
                    //Audit
            ...
            try {
                ...
                if ($charge['status']=="succeeded") {
                    ...
                    foreach($cart as $checkout){
                        ...
                        $sav['charge']=$charge=($checkout->total*$userTax->product_charge/100+($userTax->product_chargep));
                    ...
                    $merchant->revenue=$merchant->revenue+$total-($total*$userTax->product_charge/100+($userTax->product_chargep));
                    $merchant->save();
                    //Charges
                    $chargex['user_id']=$up_mer->id;
    ...
    //Invoice
        public function invoice()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_invoice_page");
            $data['title']='Invoices';
        ...
        public function Paidinvoice($id)
        {
            ...
            $userTax = UserTax::getUserTax($up_mer->id);
            $chargeVal=$data->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
            if($up_mer->balance>$chargeVal || $up_mer->balance==$chargeVal){
                $up_mer->balance+=$data->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                $up_mer->save();
                $data->status = 1;
                $data->charge=$data->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                $data->save();
                //Charges
                $charge['user_id']=$data->user_id;
                $charge['ref_id']=$data->ref_id;
                $charge['amount']=$data->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                $charge['log']='Charges for invoice #' .$data->ref_id;
                Charges::create($charge);

                $his['user_id']=$data->user_id;
                $his['amount']=$data->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                $his['ref']=$data->ref_id;
                $his['main']=0;
                $his['type']=1;
                History::create($his);
        ...
        private function getInvoicePaymentDetails($id, $data) {
            $data['invoice']=$invoice=Invoice::whereRef_id($id)->first();
        ...
        public function cardViewinvoice($id) {
            ...
            try {
                ...
                if(count($check)>0){
                    $data = $this->getInvoicePaymentDetails($id, $data);
                }else{
                    throw new \Exception('Invalid invoice');
                }

                $data['installments'] = CreditcardFeeTable::getUserInstallmentsTable($data['invoice']->amount, $data['invoice']->user_id);
                
            } catch (\Exception $ex) {
                $data['title']='Error Message';
                return view('user.merchant.error', $data)->withErrors($ex->getMessage());
            }
        ...
        public function boletoViewinvoice($id) {
            $data['lang'] = parent::getLanguageVars("user_invoice_page");
            try {
                Session::put('pay-type', 'boleto');
                $check=Invoice::whereref_id($id)->get();
        ...
        public function pixViewinvoice($id) {
            $data['lang'] = parent::getLanguageVars("user_invoice_page");
            try {
                Session::put('pay-type', 'pix');
                $check=Invoice::whereref_id($id)->get();
        ...
        public function stripeViewinvoice(Request $request, $id, $ref_id)
        {
            ...
            $currency=Currency::whereStatus(1)->first();
            $userTax = UserTax::getUserTax($ext->user_id);
            $amount=$ext->total-($ext->total*$userTax->invoice_charge/100+($userTax->invoice_chargep));
            $gate = Gateway::find(103);
            $stripe = new StripeClient($gate->val2);
            try {
                $charge=$stripe->paymentIntents->retrieve($request->input('payment_intent'));
                if ($charge['status']=="succeeded") {
                    ...
                    $up_mer->save();
                    $ext->status=1;
                    $ext->charge=$amount*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                    $ext->save();
                    $chargex['user_id']=$merchant->user_id;
                    $chargex['ref_id']=$ref_id;
                    $chargex['amount']=$ext->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                    $chargex['log']='Charges for invoice #' .$id;
                    Charges::create($chargex);
                    $his['user_id']=$merchant->user_id;
                    $his['amount']=$ext->total-($ext->total*$userTax->invoice_charge/100+($userTax->invoice_chargep));
                    $his['ref']=$ref_id;
                    $his['main']=0;
                    $his['type']=1;
                    $his['stripe_id']=$charge['id'];
                    $his['charge']=$ext->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                    History::create($his);
                    //Change status to successful
                    $change=Transactions::whereref_id($ref_id)->first();
                    $change->status=1;
                    $change->charge=$ext->total*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                    $change->save(); 
                    if($set->email_notify==1){
        ...
        */
        ...
        public function payByBancoBrasilCallback(array $paymentInfo) {
            //exit(print_r($paymentInfo));
            $txid = $paymentInfo["txid"];
            $valorRecebido = $paymentInfo["valor"];
            ...
            if ($invoice != null) {
                ...
                $amount= $invoice->total-($invoice->total*$userTax->invoice_charge/100+($userTax->invoice_chargep));
                $xtoken='INV-'.str_random(6);
                ...
                $sav['ref_id']=$xtoken;
                $sav['type']=3;
                $sav['amount']=$invoice->total-($invoice->total*$userTax->invoice_charge/100+($userTax->invoice_chargep));
                $sav['sender_id']=null;
                $sav['receiver_id']=$invoice->user_id;
                $sav['invoice_id']=$invoice->id;
                $sav['payment_link']=$invoice->id;
                $sav['payment_type']='pix';
                $sav['pix_end_to_end_id']= $txid;
                $sav['pix_callback']= json_encode($paymentInfo);
                $sav['ip_address']=user_ip();
                ...
                if ($valorRecebido >= $invoice->total) {
                    $invoice->status=1;
                    $invoice->charge=$amount*$userTax->invoice_charge/100+($userTax->invoice_chargep);
                    $invoice->save();
                }
                ...
                //Notify Users
                if($set->email_notify==1){
                    send_invoicereceipt($invoice->ref_id, 'pix', $xtoken);
                }
    ...
    //Merchant
        public function merchant()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_merchant_page");
            $data['title']='Merchant';
        ...
        */
        ...
        {    
            ...
            $data['lang'] = parent::getLanguageVars("user_transfer_pages");
            $data['title']='Error Message';
    ...
    //Fund account
        ...
        public function card(Request $request) {
            try {
                ...
                $gate = \App\Models\UserGateway::getDefaultGateway(\App\Models\UserGateway::CREDIT_CARD, $userId);
                ...
                $depo['secret'] = $secret;
                $depo['status'] = 0;
                $depo['client_name'] = $holder;
                $depo['client_document'] = $holderDocument;
                $deposit = Deposits::create($depo);
                ...
                $cielo = new Cielo_lib();
                $depositResult = $cielo->createDepositCreditCardTransaction($deposit, $cardData);
                
                $success = Array(
                    "success" => true,
                    "message" => "Pagamento recebido com sucesso! Aguarde para ser redirecionado.",
                    "redirect" => route('user.dashboard'),
                    "reference" => $depositResult->txr,
                    "total" => $depositResult->amount,
                    "status" => Deposits::getDepositStatus($depositResult->status),
                    "transaction" => Array(
                        "id" => $depositResult->creditcard_payment_id,
                        "amount" => $depositResult->amount,
                        "status" => $depositResult->creditcard_status,
                        "description" => $depositResult->creditcard_status_description,
                        "method" => $depositResult->payment_type,
                    )
                );
    ...
    public function withdraw() {
        ...
        $data['lang'] = parent::getLanguageVars("user_profile_pages");
        $data['title'] = 'Solicitar Saque';

        $today = date("Y-m-d");

        //$data['bank']=Bank::whereUser_id(Auth::guard('user')->user()->id)->get();

        $preparedWithdraw = Withdraw::whereUser_id(Auth::guard('user')->user()->id)->take(60)->latest()->get()->last();
        if ($preparedWithdraw) {
            $data['withdraw'] = Withdraw::whereUser_id(Auth::guard('user')->user()->id)->where("id", ">=", $preparedWithdraw->id)->orderBy('id', 'DESC')->paginate(6);
        } else {
            $data['withdraw'] = Withdraw::whereUser_id(Auth::guard('user')->user()->id)->orderBy('id', 'DESC')->paginate(6);
        }
        
        
        $withdrawResults=Withdraw::whereuser_id(Auth::guard('user')->user()->id)
                                    ...
                                    /*
        $data['pending']=Withdraw::whereStatus(0)
                                    ->whereuser_id(Auth::guard('user')->user()->id)
                                    ->where("paid_at", ">=", "{$today} 00:00:00")
                                    ->where("paid_at", "<=", "{$today} 23:59:59")
                                    ->sum('amount');
                                    */
        ...
        $data['received'] = 0;   
        $data['pending'] = 0;                      
        foreach($withdrawResults as $w) {
            
            if ($w->status == 1) {
                $data['received'] = $w->amount;
            } else if ($w->status == 0) {
                $data['pending'] = $w->amount;
            }
        }

        $set = Settings::first();
        $compliance = Compliance::where("user_id", Auth::guard('user')->user()->id)->first();
        $data['withdrawLimits'] = Compliance::validateWithdrawLimit($compliance, 0, $set);
        ...
        public function sendWithdraw() {
            ...
            try {
                $amount = grava_money(request("withdraw_amount"));
                $pixkey = request("pix_key");
                $pixkeyType = request("pix_key_type");

                $description = "Solicitação de saque do usuário.";
                $userId = Auth::guard('user')->user()->id;

                $withdraw = Withdraw::createWithdrawal($userId, $amount, $pixkey, $pixkeyType, $description);
        ...
        public function withdrawsubmit(Request $request) {
            ...
            $userTax = UserTax::getUserTax(Auth::guard('user')->user()->id);
            $amount=$request->amount+(($request->amount*$userTax->withdraw_charge/100+($userTax->withdraw_chargep))*$rl);
            ...
            if (!in_array($method, Array("BANK_TRANSFER", "PIX"))) {
                return back()->with('alert', 'Invalid bank transfer type.');
            }
            
            if ($method == "PIX") {
                
                if (empty($pixkey)) {
                    return back()->with('alert', 'For PIX transfer you must inform the PIX KEY.');
                }
                
                if (!in_array($pixkeyType, Array("CPF", "CNPJ", "EMAIL", "PHONE", "EVP"))) {
                    return back()->with('alert', 'Invalid PIX KEY TYPE.');
                }
            ...
            if($request->amount>(($request->amount*$userTax->withdraw_charge/100+($userTax->withdraw_chargep))*$rl)){
                    ...
                    if($user->business_level==1){
                        ...
                        if($total<$set->withdraw_limit || $set->withdraw_limit==$total){
                            ...
                            if($t_sub>$request->amount){
                                ...
                                $sav['user_id']=Auth::guard('user')->user()->id;
                                $sav['reference']=$token;
                                $sav['amount']=$request->amount-$t_sub-(($request->amount*$userTax->withdraw_charge/100+($userTax->withdraw_chargep))*$rl);
                                $sav['charge']=($request->amount*$userTax->withdraw_charge/100+($userTax->withdraw_chargep))*$rl;
                                $sav['status']=0;
                                $sav['type']=1;
                                $sav['bank_id']=$bank->id;
                                ...
                                $charge['user_id']=$user->id;
                                $charge['ref_id']=$token;
                                $charge['amount']=($request->amount*$userTax->withdraw_charge/100+($userTax->withdraw_chargep))*$rl;
    ...
    //Transfer Money
        public function Receivedpay($id)
        {
            ...
            $user=$data['user']=User::whereAccount_id($trans->temp)->first();
            $trans->status=1;
            $trans->save();
        ...
        public function submitownbank(Request $request) {
            ...
            if (!is_numeric($request->amount) || !($request->amount > 0)) {
                return back()->with('alert', 'O valor da transferência é inválido.');
            }

            $check=User::whereAccount_id(strtolower($request->account_id))->first();
            if (!$check) {
                return back()->with('alert', 'Este número de conta não existe ou está bloqueado em nosso sistema!');
            }

            $set=Settings::first();
            $currency=Currency::whereStatus(1)->first();
            $userTax = UserTax::getUserTax(Auth::guard('user')->user()->id);
            $amountx=$request->amount+($request->amount*$userTax->transfer_charge/100);
            $user=$data['user']=User::find(Auth::guard('user')->user()->id);
            if($user->email!=$request->email){
                    if($user->balance >= $amountx){
                        ...
                        if($user->status==0){
                            $trans=User::whereAccount_id($request->account_id)->first();
                            $sav['ref_id']=$token;
                            $sav['amount']=$request->amount;
                            $sav['charge']=($request->amount*$userTax->transfer_charge/100+($userTax->transfer_chargep));
                            $sav['sender_id']=Auth::guard('user')->user()->id;
                            ...
                            $charge['user_id']=$user->id;
                            $charge['ref_id']=$token;
                            $charge['amount']=$request->amount*$userTax->transfer_charge/100+($userTax->transfer_chargep);
                            $charge['log']='Charges for transfer #' .$token;
                            Charges::create($charge);
                            $his['user_id']=$user->id;
                            $his['amount']=$request->amount+($request->amount*$userTax->transfer_charge/100+($userTax->transfer_chargep));
                            $his['ref']=$token;
                            $his['main']=1;
                            $his['type']=2;
                            History::create($his);                                
                            ...
                            return redirect()->route('user.transfer')->with('success', 'Transferência enviada com sucesso!');
                        }else{
                            $data['title']='Error Message';
                            return view('user.merchant.error', $data)->withErrors('An Error Occured');
                        }
                        ...
                        return back()->with('alert', 'Você não tem dinheiro suficiente na conta');
                    }
            }else{
                return back()->with('alert', 'Você não pode transferir dinheiro para a mesma conta.');
            }
        }   
    ...
    //Request Money
        public function request()
        {
            ...
            $data['adminbank']=Adminbank::whereId(1)->first();
            $data['request']=Requests::whereuser_id(Auth::guard('user')->user()->id)->orWhere('Account_id', Auth::guard('user')->user()->account_id)->orderby('id', 'desc')->paginate(6);
        ...
        public function submitrequest(Request $request)
        {
            ...
            if($user->email!=$request->email){
                if(count($check)>0){
                    ...
                    return redirect()->route('user.request')->with('success', 'A solicitação foi enviada com sucesso');
                }else{
                    return back()->with('alert', 'Usuário não encontrado.');
                }
            }else{
                return back()->with('alert', 'Você não pode solicitar dinheiro da mesma conta.');
            }
        }
        public function Sendpay($id)
        {
            ...
            $userTax = UserTax::getUserTax($sender->id);
            $amount=$trans->amount+($trans->amount*$userTax->transfer_charge/100+($userTax->transfer_chargep));
            if($trans->status==0){
                if($sender->balance>$amount || $sender->balance==$amount){
                    ...
                    $charge['amount']=$trans->amount*$userTax->transfer_charge/100+($userTax->transfer_chargep);
                    ...
                    return back()->with('alert', 'Você não tem saldo suficiente para essa transação!');
                }
            }else{
                $data['title']='Error Message';
                return view('user.merchant.error', $data)->withErrors('Já foi pago!');
            }
            return redirect()->route('user.request')->with('success', 'Transferência enviada com sucesso!');
        }        
    ...
    //Payment link
        public function sclinks()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_link_page");
        ...
        public function submitsinglecharge(Request $request) {
            ...
            if ($amount > 0) {
                $total = $amount;
                $charge = number_format((($amount * $userTax->single_charge / 100) + $userTax->single_chargep), 2, ".", "");
            ...
            $sav['charge']= $charge;
            $sav['total']= $total;
            $sav['currency']= $currency->name;
            $sav['currency_id']= $currency->id;
            $sav['name']= $request->name;
        ...
        public function cardscviewlink($id) {
            ...
            try {
                ...
                if(count($check)>0){
                    $data = $this->getSingleChargePaymentDetails($id, $data);
                }else{
                    throw new \Exception('Invalid invoice');
                }
            } catch (\Exception $ex) {
        ...
        public function pixscviewlink($id) {
            $data['lang'] = parent::getLanguageVars("user_link_page");
            try {
                Session::put('pay-type', 'pix');
                $check=Paymentlink::whereref_id($id)->get();
        ...
        public function boletoscviewlink($id) {
            $data['lang'] = parent::getLanguageVars("user_link_page");
            try {
                Session::put('pay-type', 'boleto');
                $check=Paymentlink::whereref_id($id)->get();
        ...
        private function getSingleChargePaymentDetails($id, $data) {
            $data['invoice']=$invoice=Invoice::whereRef_id($id)->first();

            $key=Paymentlink::whereref_id($id)->first();

            if($key->user->status > 0){
                throw new \Exception("Sua conta não está habilitada para pagamento.");
            }

            if($key->status > 0){
                throw new \Exception("O link não está mais disponível para pagamento.");
            }

            if($key->active<1){
                throw new \Exception("O link não está mais ativo para pagamento.");
            }
            
            $data['link']= $link=Paymentlink::whereref_id($id)->first();
            $data['merchant']= $user=User::find($link->user_id);
            $set=Settings::first();
            $data['title']="Link de Pagamento - ".$link->name;
        ...
        public function dpviewlink($id) {
            ...
            $userTax = UserTax::getUserTax($link->user_id);
            $data['userTax']=$userTax;
        ...
        public function carddpviewlink($id) {
            try {
                Session::put('pay-type', 'card');
                $data = $this->getDonationPaymentDetails($id);
            } catch (\Exception $ex) {
                $data['title']='Error Message';
                return view('user.merchant.error', $data)->withErrors($ex->getMessage());
            }
        ...
        public function boletodpviewlink($id) {
            try {
                Session::put('pay-type', 'boleto');
                $data = $this->getDonationPaymentDetails($id);
            } catch (\Exception $ex) {
        ...
        public function pixdpviewlink($id) {
            try {
                Session::put('pay-type', 'pix');
                $data = $this->getDonationPaymentDetails($id);
            } catch (\Exception $ex) {
        ...
        private function getDonationPaymentDetails($id) {
            $data['lang'] = parent::getLanguageVars("user_link_page");
            $check=Paymentlink::whereref_id($id)->get();

            if(!count($check) > 0) {
                throw new \Exception("Link inválido ou inexistente.");
            }
    ...
    //Settings
        ...
        public function submitcompliance(Request $request)
        {
            ...
            $com->mouth_revenue=number_format($request->mouth_revenue, 2, ".", "");
            $com->patrimony=number_format($request->patrimony, 2, ".", "");
            $com->neighborhood=$request->neighborhood;
            $com->mother_name=$request->mother_name;
            $com->address_type=$request->address_type;
            $com->address_number=$request->address_number;
            $com->address_complement=$request->address_complement;
            $com->address_zipcode=$request->address_zipcode;
            $com->phone_country_code=$request->phone_country_code;
            $com->phone_type=$request->phone_type;
            $com->address_city=$request->address_city;
            $com->address_state=$request->address_state;
        ...
        {
            $data=User::findOrFail(Auth::guard('user')->user()->id);
            $data->public_key='PUBLIC-AZZOPAY-RESET-'.str_random(32);        
            $data->secret_key='SECRET-AZZOPAY-RESET-'.str_random(42);        
            $data->save();
            return back();
        }


        public function registerCallbackUrl(Request $request) {
            try {

                $data = User::findOrFail(Auth::guard('user')->user()->id);
                $data->callback_url = request('url');         
                $data->save();

                return response()->json(
                    ['message' => "URL alterada com sucesso!", "success" => true],
                    200
                );
            } catch(\Exception $ex) {
                return response()->json(['success' => false, "message" => $ex->getMessage()], 200);
    ...
    //Bank functions 
        public function bank()
        {
            ...
            $data['lang'] = parent::getLanguageVars("user_bank_accounts");
        ...
        {
            $langLayout = parent::getLanguageVars("login_layout_pages");
            $langPage = parent::getLanguageVars("user_bank_accounts");
            $data['lang'] = array_merge($langLayout, $langPage);
        ...
        public function Updatebank(Request $request)
        {
            ...
            if($set->stripe_connect==1){
                ...
                $bank->bank_id=$request->bank;
                $bank->acct_no=$request->acct_no;
                $bank->agency_no=$request->agency_no;
                $bank->account_document=$request->account_document;
                $bank->acct_name=$request->acct_name;
                $bank->account_type=$request->account_type;
                $bank->routing_number=$request->routing_number;
                $bank->save();
        ...
        {  
            ...
            if($set->stripe_connect==1){
                ...
                $data['agency_no']=$request->agency_no;
                $data['acct_no']=$request->acct_no;
                $data['account_document']=$request->account_document;
                $data['acct_name']=$request->acct_name;
                $data['account_type']=$request->account_type;
                $data['routing_number']=$request->routing_number;
                $data['bank_id']=$request->bank;
    ...
    //Plans & Subscription
        ...
        {
            $data['lang'] = parent::getLanguageVars("user_plans_page");
            $data['title']='Plans';
    ...
    //Transaction Logs
        public function transactions() {
            ...
            $data['lang'] = parent::getLanguageVars("user_transaction_pages");
