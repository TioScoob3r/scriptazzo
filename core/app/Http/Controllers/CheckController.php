...
class CheckController extends Controller
{
    ...
    public function vcard(){
        $data['lang'] = parent::getLanguageVars("admin_virtual_page");
    ...
    public function bpay(){
        $data['lang'] = parent::getLanguageVars("admin_bill_page");
    ...
    {
        $data['lang'] = parent::getLanguageVars("admin_dashboard_page");
    ...
    public function ChangeEnroller(Request $request) {
        try {
            ...
            if (empty($code)) {
                throw new \Exception("Você deve informar o E-mail ou id do patrocinador.");
            }

            $enroller = null;
            if (is_numeric($code)) {
                $enroller = User::where("id", $code)->first();
            } else {
                $enroller = User::where("email", $code)->first();
            }
            if (!$enroller) {
                throw new \Exception("Não foi encontrado um patrocinador com os dados informados.");
            }

            if (!is_numeric($userId) || !($userId > 0)) {
                throw new \Exception("Você deve informar a identificação do usuário.");
            }
            ...
            $user->enroller = $enroller->id;
            $user->save();

            return response()->json(
                ["message" => "Patrocinador alterado com sucesso!", "success" => true],
                 200
            );
        } catch(\Exception $ex) {
    ...
    public function SearchEnroller(Request $request) {
        try {
            ...
            return response()->json(
                ['name' => "{$enroller->first_name} {$enroller->last_name}", "success" => true],
                 200
            );
        } catch(\Exception $ex) {
            return response()->json(['success' => false, "message" => $ex->getMessage()], 200);
