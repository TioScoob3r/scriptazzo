...
class CieloController extends Controller {
    ...
    public function index(Request $request) {
        ...
        try {
            ...
            DB::table('logs_callbacks')->insert([
                'logs' => json_encode($info),
                'type' => 'request',
            ...
            if($changType == 1) {
                ...
                $invoice = DB::table('invoices')->where('ref_id', $merchantOrderId);
                $invoice->update([
                    'status' => 1
                ]);

                $user = DB::table('users')->where('id', $invoice->first()->user_id);
                $user->update([
                    'balance' => $user->first()->balance + $invoice->first()->amount
                ]);
                $invoice->update([
                    'status' => 1
                ]);
            ...
            DB::table('logs_callbacks')->insert([
                'logs' => $ex->getMessage() . ' '. $ex->getFile() . ' '. $ex->getLine(),
                'type' => 'error',
                'created_at' => date('Y-m-d H:i:s')
            ]);
