                ...
                <div class="card bg-white border-0 mb-0">
                    @if($stripe->status==1)
                        <div class="card-header" id="headingOne">
                            <h4 class="h3 mb-0 font-weight-bolder">{{$lang["merchant_card"]}}</h4>
                        </div>
                            <div class="card-body">
                                ...
                                @csrf
                                    ...
                                    <div class="text-center mt-5">
                                        <button type="submit" class="btn btn-neutral btn-block"><i class="fad fa-external-link"></i> {{$lang["merchant_pay"]}}</button><br>
                                    </div>
                                </form>
                            </div>
                        <hr>
                    ...
                    <div class="card-header">
                        <h4 class="h3 mb-0 font-weight-bolder">{{$lang["merchant_pay_with_account"]}}</h4>
                    </div>
                    <div class="card-body text-center">
                        @if (Auth::guard('user')->check())
                            <form method="post" role="form" action="{{route('pay.merchant')}}">
                            @csrf
                            <h4 class="mb-0">{{$lang["merchant_account_balance"]}}</h4>
                        ...
                        @else
                            ...
                            <h3 class="mb-3 text-muted">{{$lang["merchant_login_to_make_payment"]}}</h3>
                            <a href="{{route('login')}}" class="btn btn-neutral btn-block">{{$lang["merchant_login"]}}</a>
                        @endif
                        </div>
                    </div>
                </div>
            <div class="col-md-4 order-first-sm">
                <div class="card">
                    <div class="card-body">          
                        ...
                        <small class="text-muted">{{$lang["merchant_by"]}} {{$merchant->business_name}} {{$lang["merchant_on"]}} {{date("h:i:A j, M Y", strtotime($link->created_at))}}</small>
