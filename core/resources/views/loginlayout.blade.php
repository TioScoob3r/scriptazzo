    ...
    <nav id="navbar-main" class="navbar navbar-horizontal navbar-transparent navbar-main navbar-expand-lg navbar-dark">
      ...
      @if (Auth::guard('user')->check()) 
        <a class="navbar-brand text-dark" href="{{route('user.logout')}}">
          <span style="color:white;"><i class="fad fa-sign-out"></i>  {{$lang["login_layout_logout"]}}</span>
        </a>
      ...
      @else
        <a class="navbar-brand text-dark" href="{{url('/')}}">
          <span style="color:white;"><i class="fad fa-arrow-left"></i>  {{$lang["login_layout_home"]}}</span>
        </a>
        <div class="navbar-collapse navbar-custom-collapse collapse" id="navbar-collapse">
          <div class="navbar-collapse-header">
            <div class="row">
...
"use strict"
function check(){
    if ($("#seeAnotherField").val() == "{{$lang['login_layout_registered_businesss']}}") {
      $('#otherFieldDiv').show();
      $('#otherField').attr('required', '');
      $('#otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');  
      $('#6xxotherFieldDiv').show(); 
      $('#6xxotherField').attr('required', '');
      $('#6xxotherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}'); 
      $('#fFieldDiv').show();
      $('#fField').attr('required', '');
      $('#fField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');         
      $('#ffFieldDiv').show();
      $('#ffField').attr('required', '');
      $('#ffField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');    
      $('#otherFieldDiv1').show();
      $('#otherField1').attr('required', '');
      $('#otherField1').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');      
      $('#otherFieldDiv2').show();
      $('#customFileLang').attr('required', '');
      $('#customFileLang').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');  
      $('#otherFieldDiv3').hide();
      ...
      $('#1otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');      
      $('#2otherField').attr('required', '');
      $('#3otherField').attr('required', '');
      $('#3otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');      
      $('#4otherField').attr('required', '');
      $('#4otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');
      $('#5otherField').attr('required', '');
      $('#5otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');
      $('#6otherField').attr('required', '');
      $('#6otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');      
      $('#6xotherField').attr('required', '');
      $('#6xotherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');      
      $('#60otherField').attr('required', '');
      $('#60otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');
      $('#60xotherField').attr('required', '');
      $('#60xotherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');
      ...
      $('#7otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');      
      $('#70otherField').attr('required', '');
      $('#70otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');
      $('#8otherField').attr('required', '');
      $('#8otherField').attr('data-error', '{{$lang["login_layout_this_field_is_required"]}}');
